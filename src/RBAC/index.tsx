...
const RBAC = ({ WrapperElem, LoadingElem, authData }: RBAC_PROPS) => {
  const Validator = () => {
    const { pathname } = useLocation();
    const navigate = useNavigate();
    const [shouldRender, setShouldRender] = useState<boolean>(false);
    const { hasToken, onUnauthorizedPageRequest, roles, routes, redirects } =
      authData;
    ...
    return shouldRender ? (
      ...
      <>{LoadingElem ? <LoadingElem /> : <DefaultLoader />}</>
    );
  };
